...
from ..context_rich import RichContextBuilder, RichContextPolicy
from ..state import ContextBudgetExceeded
from .validation import dumps, number
...
class ExperimentContext:
    def __init__(self, world, policy: RichContextPolicy | None = None):
        ...
        self.builder = RichContextBuilder(world, self.policy)

    def build(self, *, now: float, **kwargs) -> dict:
        now = number(now)
        result = self.builder.build(**kwargs)
