...
`physical_harness/world/memory/spatial_views.py` adds:
...
`physical_harness/world/spatial_scene.py` adds an interface for geometry-heavy moments without turning the whole episode into a dense reconstruction problem.
...
`python -m physical_harness.integrations.experiment export-replay` consumes that snapshot as
