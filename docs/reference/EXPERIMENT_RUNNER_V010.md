...
python -m physical_harness.integrations.experiment demo --output /tmp/physical-demo-new
python -m physical_harness.integrations.experiment doctor \
  --config experiments/fixtures/configs/doctor.fixture.json
python -m physical_harness.integrations.experiment export-replay \
...
Copy `configs/behavior/pilot.template.json` to a private run configuration. It intentionally fails validation until placeholders are replaced.
...
```bash
python -m physical_harness.integrations.experiment doctor --config /private/pilot.json
```
...
The shipped `physical_harness.integrations.experiment.existing_bridge:driver_factory` adapts that object into `NativeBindings`; no edit to the experiment package is needed. Alternatively, your factory may return `NativeBindings` directly and be named in the native command.
...
```bash
python -m physical_harness.integrations.experiment run \
...
```bash
python -m physical_harness.integrations.experiment evaluate-replay \
...
- https://github.com/AranKomat/physical-agent-harness/blob/72fbfd15b5fe547e8f447884236ac5edfb27ee96/physical_harness/core/runtime.py
- https://github.com/AranKomat/physical-agent-harness/blob/72fbfd15b5fe547e8f447884236ac5edfb27ee96/physical_harness/reasoning/verification.py
- https://github.com/AranKomat/physical-agent-harness/blob/72fbfd15b5fe547e8f447884236ac5edfb27ee96/physical_harness/reasoning/visual_verifier.py
- https://github.com/AranKomat/physical-agent-harness/blob/72fbfd15b5fe547e8f447884236ac5edfb27ee96/physical_harness/world/memory/integration.py
