...
No model weights are loaded by importing `physical_harness.planning.tasks`. No provider client, tracker model, simulator, or actuator is started on import.
...
```bash
/path/to/isolated/python -m physical_harness.integrations.sam \
...
python -m pytest -q
ruff check .
python -m physical_harness.planning.tasks demo --output runs/situated-fixture-new
```
...
```python
from physical_harness.planning.tasks import (
    CatalogActionHandler, GraphSession, Handler, IdentityLedger,
    enforce_extra_step_checks,
)
from physical_harness.execution.actions import ActionExecutor
...
[corrected incremental shadow run](../experiments/2026-09-22/SAM31_INCREMENTAL_SHADOW_20260922.md),
[capacity and timing](../experiments/2026-09-22/SAM31_CAPACITY_AND_TIMING_20260922.md), and
[latency path](../experiments/2026-09-22/PERCEPTION_LATENCY_PATH_20260922.md).
