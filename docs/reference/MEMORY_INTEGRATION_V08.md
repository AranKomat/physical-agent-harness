...
```text
physical_harness/world/memory/integration.py
tests/world/memory/test_integration.py
...
`physical_harness/world/memory/selection.py` so every existing `RuntimeEvent` value is
