...
| 13 | Controlled scope now includes mixed attributes, seed, distractor, tracker reset, native-instance, partial/full occlusion, task/object transfer, and one Halloween candle subgoal. The native-instance cohort passed ID 302 but failed dark ID 303. Fresh ID 304 safely aborted under full occlusion. Soda-can/`picking_up_trash` completed a separately verified 50.010 mm lift. The scripted Halloween candle then completed 30/30 actions with 50.004 mm FK/evaluator lift, but SAM found 0/3 candle prompts and the settled Astra visual response was lost to a fixed local artifact-directory bug; the preregistered trial therefore did not pass. Broader layouts, placements, complete tasks, and benchmark-valid held-out workflows remain open. |
...
4. Preserve the [scripted-lift power baseline](2026-09-25/PHASE14_SCRIPTED_LIFT_POWER_BASELINE_20260925.md):
...
5. Keep Sol as the default executive. The selective rule has corrected one frozen
   ...
   prospective Sol error next; keep Astra shadow-only until that broader evidence exists.
...
8. Keep the [image-quality and full-occlusion gate](2026-09-25/PHASE13_IMAGE_QUALITY_AND_OCCLUSION_GATE_20260925.md)
...
10. Preserve the [Halloween candle lift](2026-09-25/PHASE13_HALLOWEEN_CANDLE_LIFT_20260925.md)
   as positive physical-effect evidence but a failed overall protocol. The candle
   moved 50.004 mm under both FK and evaluator scoring; the independent visual
   verdict is unavailable, and the identical lift must not be retried merely to
   replace that missing artifact.
