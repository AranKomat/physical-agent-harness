...
| 9. Compiler manipulation | Phase 9A is complete for proposal-only research: exact simulated hand export, pinned `sweep_volume_v2` conditioning, reference geometric comparison, numerical IK/rejection screens and one retained offline candidate. A 35-action native empty-hand close/reopen probe tracked 49.999 mm -> 0.00019 mm -> 49.999 mm with 1.32 um maximum asymmetry; no execution | Do not generate more proposals. Production grasp-frame/contact authority remains unqualified, and the surviving candidate requires torso/base staging through unseen space. Phases 9B--9D return to the Phase 5--7 legal current-clearance gate |
