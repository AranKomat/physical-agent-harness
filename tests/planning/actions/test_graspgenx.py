...
from physical_harness.integrations.graspgenx import (
    AUDITED_REVISION,
    GraspGenXAdapter,
    LocalAssets,
    load_local,
...
def test_importing_adapter_does_not_import_upstream():
    import subprocess
    import sys
    command = 'import sys; import physical_harness.integrations.graspgenx; assert "graspgenx" not in sys.modules; assert "torch" not in sys.modules'
...
def local_asset_fixture(tmp_path, f):
    ...
    from physical_harness.core.actions import digest
...
def test_loader_validates_everything_before_mock_upstream_import(tmp_path, monkeypatch):
    ...
    import physical_harness.integrations.graspgenx as module
    f = Fixture()
