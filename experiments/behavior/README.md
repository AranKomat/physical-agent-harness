...
Luna is retired from active configuration by operator decision; its historical
receipts remain intact. GLM remains the inventory candidate. A later matched
Sol-versus-Astra comparison must use the same observations, prompts, reasoning
effort, action budgets and verification, with separately authorized API calls.
Public demos motivate that comparison but do not establish a winner here.
