...
Sparse posed RGB-D keyframes and explicit observation-coverage records are now
available through an opt-in `MemorySidecar` path. They remain historical shadow
evidence and never update `WorldState` or live executive input. An on-demand
